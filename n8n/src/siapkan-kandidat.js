...
const prompt = [
  ...
  `Pemainnya: ${roster}.`,
  'Untuk SETIAP gambar jawab tiga hal. Pakai kostum, warna rambut, dan ciri tokoh sebagai',
  ...
  '[{"i":1,"tokoh":["nama pemain","nama karakter"],"utama":"nama","wajah":true,"adegan":"maks 8 kata"}]',
].join('\n');
...
const ROMBONGAN = 8;
const gambar = [];
for (let i = 0; i < kecil.length; i += ROMBONGAN) {
  const sepotong = await Promise.all(
    kecil.slice(i, i + ROMBONGAN).map((u) =>
